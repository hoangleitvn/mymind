---
platform: substack
created: 2025-12-10
updated: 2025-12-20
status: launching
positioning: Engineering leader writing about building things that last
url: buildsthatlast.substack.com
custom_domain: buildsthatlast.com (pending)
next_review: 2026-03-20
---

# Builds That Last - Substack Profile

## Summary

**Status:** Launching December 2025

| Section | Status | Priority |
|---------|--------|----------|
| Bio | Done | High |
| Profile Photo | Done | High |
| Publication Name | Done | High |
| About Page | Ready | High |
| Sections | Defined | High |
| Start Here Page | Pending | Medium |
| Principles Page | Pending | Low |

---

## Brand Architecture

```
Hoang Le (person, account owner @hoangleitvn)
├── Builds That Last (newsletter - thought leadership)
├── INNOMIZE (company - services)
└── Mize Studio
    ├── MizeForge (tools)
    └── MizeStack (infrastructure)
```

**Builds That Last** is personal thought leadership, separate from company brands.

---

## Content Architecture

### Sections (Categories)

Substack sections allow readers to subscribe to specific content types.

| Section | Slug | Description | Content |
|---------|------|-------------|---------|
| **Foundation** | foundation | Core philosophy, principles, manifestos | Evergreen essays on what matters |
| **Leadership** | leadership | Engineering leadership lessons | Team dynamics, growth, decisions |
| **Building** | building | Product development, shipping, execution | Practical insights, process |
| **Systems** | systems | Architecture, technical decisions | Trade-offs, patterns, design |
| **Field Notes** | field-notes | Shorter observations, quick takes | 500-800 word pieces |

### Pages

| Page | Purpose | Status |
|------|---------|--------|
| **About** | Who I am, what subscribers get | Ready |
| **Start Here** | Curated intro for new readers | Pending |
| **Principles** | Core beliefs with links to posts | Pending |

### Post Types

| Type | Length | Frequency | Typical Section |
|------|--------|-----------|-----------------|
| **Manifesto** | 2000-3000 words | Rare (founding pieces) | Foundation |
| **Deep Dive** | 1500-2500 words | 2x/month | Leadership, Building, Systems |
| **Essay** | 800-1200 words | Weekly | Any |
| **Field Note** | 400-800 words | As needed | Field Notes |

### Content Flow (from Platform Strategy)

```
Idea → BTL Substack (full essay)
         ↓
    LinkedIn (teaser post + link to BTL)
         ↓
    3-5 atomic LinkedIn posts (key insights)
```

**BTL exclusivity:** 80% exclusive to BTL, 20% republished on LinkedIn Articles

---

## Bio

### Recommended

```
Engineering leader & founder. 15+ years building products. Helping founders go from zero to live users. Writing about engineering leadership, product development, and builds that last.
```

**Character count:** ~180

### Why This Works
- Clean and scannable
- Positions as practitioner, not guru
- "Builds that last" ties to publication name and core principle
- Works for broader audience (not just founders seeking CTOs)

### Keywords Included
- engineering leader, founder
- building products
- founders, zero to live users
- engineering leadership, product development

---

## Publication Name

### Final Choice

**Name:** Builds That Last

**Alias:** buildsthatlast.substack.com

**Custom Domain:** buildsthatlast.com (to purchase)

**Backup Domain:** theshippingpractice.com (to purchase)

**Short description:** Building and shipping products with clarity. Engineering leadership, product development, and things that last.

**Publication introduction:**
Builds That Last is about building products that endure. Engineering leadership lessons, product development principles, and honest takes on architecture, teams, and execution from 15 years shipping 50+ projects. Each post comes from something I've built, broken, or fixed. Subscribe if you're an engineer becoming a leader, a founder building a technical team, or anyone tired of advice that doesn't ship.

**Character count:** ~480

**Categories:**
- Primary: Technology
- Secondary: Startups or Business

### Taglines

**Primary (covers, headers, social):**
> Everyone ships fast. Few build to last.

**Secondary (About page, intro):**
> In the AI era, shipping is easy. Building to last is what matters.

### Why This Name
- Outcome-focused: emphasizes durability and quality
- "Last" = lasting impact, things that endure
- Short, memorable (4 syllables)
- Stands alone from personal brand (hoangleitvn)
- Separate from company brands (INNOMIZE, Mize*)
- Avoids conflict with shipwithclarity.com (competitor)

### Previous Name (Deprecated)
- **Ship with Clarity** - deprecated due to domain conflict with competitor selling similar frameworks

---

## About Page (Future)

### Best Practices (Research Summary)

**The 3 Questions Readers Ask (in 60 seconds):**
1. "Who is this person?" - Credibility
2. "What will I get?" - Value
3. "Do I belong here?" - Community fit

**Must-Have Elements:**
- Photo (face-forward, builds trust)
- What subscribers get (topics, frequency, format)
- Benefits (how their life improves)
- About you (2-4 sentences, relevant expertise)
- Mission (why this exists, what problem it solves)
- Subscribe CTA (at least 2: top and bottom)

**Key Tips:**
- First 200 words matter most
- Don't link to external sites
- Define who it's NOT for
- Use subheadings for scanning

**Sources:**
- [Substack Official Guide](https://on.substack.com/p/how-to-polish-your-publications-about)
- [100 About Pages Analyzed](https://ditchthetemplates.substack.com/p/i-analyzed-100-substack-about-pages)
- [How to Make a Great About Page](https://pubstacksuccess.substack.com/p/how-to-make-a-great-about-page)

### Recommended Structure

Hi, I'm Hoang.

Subscribe if you want engineering insights that actually ship.

---

Most engineering advice sounds good in theory. It falls apart when you're under pressure to deliver.

I've spent 15 years learning what actually lasts.

50+ projects shipped across startups and enterprise. Supply chain, law tech, healthcare, public SaaS. Teams scaled from 0 to 40+ engineers. Clients across US, UK, France, Singapore, and Malaysia.

This newsletter is for engineers and founders who want to build things that last. Not things that break.

**What you'll get:**
- Engineering leadership lessons from real projects
- Product development principles that work when things go wrong
- Honest takes on architecture, teams, and shipping

Each post comes from something I've built, broken, or fixed.

**Who this is for:**
- Engineers becoming leaders
- Founders building technical teams
- Anyone tired of advice that doesn't ship

**Who this is NOT for:**
- People looking for quick hacks
- Those who want theory without tradeoffs

Subscribe to get each post in your inbox.

---

## Profile Photo

Use same professional photo as LinkedIn for consistency across platforms.

---

## Action Items

### Setup (Priority: High)

- [x] Create Substack account
- [x] Add bio
- [x] Set publication name
- [x] Upload profile photo (same as LinkedIn)
- [ ] Configure sections in Substack (Foundation, Leadership, Building, Systems, Field Notes)
- [ ] Write and publish About page
- [ ] Purchase buildsthatlast.com domain
- [ ] Configure custom domain on Substack

### Launch (Priority: High)

- [ ] Publish manifesto as Post #1 (Section: Foundation)
- [ ] Publish AI hiring article as Post #2 (Section: Leadership)
- [ ] Post LinkedIn teaser for manifesto
- [ ] Post LinkedIn teaser for AI hiring article
- [ ] Announce BTL on LinkedIn (new publication post)

### Post-Launch (Priority: Medium)

- [ ] Create Start Here page (after 3-5 posts)
- [ ] Create Principles page (after principle-related posts exist)
- [ ] Set up cross-promotion in email footer
- [ ] Add BTL link to LinkedIn profile Featured section

### Deferred

- [ ] Purchase theshippingpractice.com domain (backup) - low priority
- [ ] Evaluate Notes feature for short-form content
- [ ] Consider podcast/audio versions

---

## Content Strategy

### Content Pillars

| Pillar | Description | Maps to Section |
|--------|-------------|-----------------|
| **Engineering Leadership** | Leading teams, making decisions, growing engineers | Leadership |
| **Product Development** | Shipping, execution, founder lessons | Building |
| **Architecture & Systems** | Technical decisions, trade-offs, patterns | Systems |
| **Foundation Principles** | Core philosophy, what lasts, what matters | Foundation |
| **AI & Modern Development** | AI in practice, tools, productivity | Building, Systems |

### Themes to Cover

**Foundation (Evergreen):**
- Why foundation comes before speed
- The 80% below water (maintenance reality)
- Building for the people who come after

**Leadership:**
- Engineers becoming leaders
- Scaling teams (0 to 40+)
- Decision-making under pressure
- The knowing vs. applying gap

**Building:**
- Shipping vs. shipping well
- Product validation before building
- Working with founders
- The embedded CTO model

**Systems:**
- Architecture decisions and trade-offs
- Technical debt reality
- AI as turbo, not robot
- Evaluation and validation systems

### Publishing Cadence

| Content | Frequency | Day |
|---------|-----------|-----|
| Deep Dive or Essay | Weekly | Sunday or Monday |
| Field Note | As inspired | Any |
| LinkedIn teaser | Same day as BTL post | - |
| LinkedIn atomic posts | Throughout week | - |

### Launch Content Plan

| Order | Post | Section | Status |
|-------|------|---------|--------|
| 1 | Builds That Last Manifesto | Foundation | Ready |
| 2 | AI Hiring Article | Leadership | Ready |
| 3 | TBD | - | - |

---

## Start Here Page (Draft)

**Purpose:** Guide new subscribers to best content

### Structure

```markdown
# Start Here

New to Builds That Last? Here's where to begin.

## The Foundation
Start with these to understand what this publication is about:
- [Builds That Last: Why Foundation Comes Before Speed] - The manifesto
- [Post 2 - TBD]
- [Post 3 - TBD]

## Most Popular
[Will populate as content grows]

## By Topic
- **Engineering Leadership:** [links]
- **Product Development:** [links]
- **Architecture & Systems:** [links]

## About Me
I'm Hoang Le. 15 years building products. 50+ projects shipped. Currently running INNOMIZE, helping founders go from zero to live users.

[Read my full story →]
```

---

## Principles Page (Draft)

**Purpose:** Codify core beliefs, link to posts that expand each

### Structure

```markdown
# Principles

These are the beliefs that guide everything I write.

1. **Foundation before speed.** Real speed comes from clarity, not from typing faster.

2. **The demo always works.** It's what comes after that separates software that lasts from software that collapses.

3. **Maintenance costs more than building.** Every shortcut in building becomes a tax on every future change.

4. **AI is a turbo, not a robot.** It speeds up what you already have. Good foundation? Faster good software. Bad foundation? Faster bad software.

5. **Knowing ≠ Applying.** The gap between knowing the right way and doing the right way is where most projects fail.

6. **Build for the people who come after.** The engineer who maintains your code in 10 years is your real customer.

[More principles added as I write about them]
```

---

## Next Review

**Date:** 2026-03-20 (Quarterly)

**Focus:**
- Subscriber growth rate
- Open rates and engagement
- Top performing content/sections
- Cross-promotion effectiveness with LinkedIn
- Section subscription distribution

---

## Change Log

| Date | Changes Made |
|------|--------------|
| 2025-12-10 | Initial profile document created |
| 2025-12-11 | Rebranded from "Ship with Clarity" to "Builds That Last" due to domain conflict |
| 2025-12-11 | Updated taglines |
| 2025-12-13 | Revised About page structure |
| 2025-12-20 | Added content architecture: sections, pages, post types |
| 2025-12-20 | Added Start Here and Principles page drafts |
| 2025-12-20 | Updated status to "launching", aligned with platform strategy |

